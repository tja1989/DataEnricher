#!/usr/bin/env python3
"""
Hybrid Vector Embedder for Equipment Data
...
class HybridVectorEmbedder:
    def __init__(self):
        ...
        self.structured_model = SentenceTransformer('sentence-transformers/all-MiniLM-L6-v2')
    ...
    def process_records(self, records):
        ...
        print(f"  Generating {len(semantic_texts)} semantic embeddings...")
        semantic_embeddings = self.semantic_model.encode(semantic_texts, show_progress_bar=False, batch_size=8)
        
        print(f"  Generating {len(structured_texts)} structured embeddings...")
        structured_embeddings = self.structured_model.encode(structured_texts, show_progress_bar=False, batch_size=8)
        ...
        self.structured_index.add(np.array(structured_embeddings).astype('float32'))
    ...
    def save_indices(self, output_dir="vector_indices"):
        ...
        with open(metadata_path, 'w') as f:
            json.dump(metadata_with_stats, f, indent=2)
    ...
    def hybrid_search(self, query, k=5, alpha=0.6):
        ...
        query_semantic = self.semantic_model.encode([query])
        query_structured = self.structured_model.encode([query])
        
        # Search both indices
        sem_distances, sem_indices = self.semantic_index.search(
            np.array(query_semantic).astype('float32'), k
        )
        
        str_distances, str_indices = self.structured_index.search(
            np.array(query_structured).astype('float32'), k
        )
